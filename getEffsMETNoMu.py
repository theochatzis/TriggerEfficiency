...
class TrigMETAnalysis(Module):
    ...
    def beginJob(self,histFile=None,histDirName=None):
        ...
        self.h_metNoMu_pt_vs_pv_all = ROOT.TH2F("h_metNoMu_pt_vs_pv_all", ";p_{T}^{miss(no-#mu)} [GeV];PV;Efficiency", len(self.bins["met_pt"])-1, array.array("d", self.bins["met_pt"]), 50, 0, 100)
        self.h_metNoMu_pt_vs_pv_passed = ROOT.TH2F("h_metNoMu_pt_vs_pv_passed", ";p_{T}^{miss(no-#mu)} [GeV];PV;Efficiency", len(self.bins["met_pt"])-1, array.array("d", self.bins["met_pt"]), 50, 0, 100)

        self.h_metNoMu_pt_all = ROOT.TH1F("h_metNoMu_pt_all", "; p_{T}^{miss(no-#mu)} [GeV]", len(self.bins["met_pt"])-1, array.array("d", self.bins["met_pt"]))
        self.h_metNoMu_pt_passed = ROOT.TH1F("h_metNoMu_pt_passed", "; p_{T}^{miss(no-#mu)} [GeV];Efficiency", len(self.bins["met_pt"])-1, array.array("d", self.bins["met_pt"]))
        ...
        for path in self.signal_paths:
            ...
            self.hList[f'h_metNoMu_pt_passtrig_HLT_{path}'] = ROOT.TH1F(f'h_metNoMu_pt_passtrig_HLT_{path}', ";p_{T}^{miss} [GeV]", len(self.bins["met_pt"])-1, array.array("d", self.bins["met_pt"]))
        ...
        self.addObject(self.h_metNoMu_pt_all)
        self.addObject(self.h_metNoMu_pt_passed)
        self.addObject(self.h_metNoMu_pt_vs_pv_all)
        self.addObject(self.h_metNoMu_pt_vs_pv_passed)
    ...
    def analyze(self, event):
        ...
        # Remove the muon for MET (Create MET No Mu)
        met_px = met.pt*math.cos(met.phi)
        met_py = met.pt*math.sin(met.phi)
        mu_px = mu.pt*math.cos(mu.phi)
        mu_py = mu.pt*math.sin(mu.phi)

        metNoMu = math.sqrt((met_px + mu_px)*(met_px + mu_px)+(met_py + mu_py)*(met_py + mu_py))
        
        self.h_metNoMu_pt_all.Fill(metNoMu)
        self.h_metNoMu_pt_vs_pv_all.Fill(metNoMu, pv.npvsGood)
        ...
        for path in self.signal_paths:
            if getattr(hlt, path) == 1:
                ...
                self.hList[f'h_metNoMu_pt_passtrig_HLT_{path}'].Fill(metNoMu)
                
        if signalOR:
            self.h_pv_passed.Fill(pv.npvsGood)
            self.h_met_pt_passed.Fill(met.pt)
            self.h_met_pt_vs_pv_passed.Fill(met.pt, pv.npvsGood)
            self.h_metNoMu_pt_passed.Fill(metNoMu)
            self.h_metNoMu_pt_vs_pv_passed.Fill(metNoMu, pv.npvsGood)
...
signal_paths    = ["PFMETNoMu120_PFMHTNoMu120_IDTight",
                   "PFMETNoMu120_PFMHTNoMu120_IDTight_FilterHF",
                   ]

p=PostProcessor(".",files,cut=preselection,branchsel=None,modules=[TrigMETAnalysis()],noOut=True,histFileName="histos_METNoMuTrigNanoAOD.root",histDirName="metTrigAnalyzerNanoAOD")
p.run()
